...
```bash
git clone https://github.com/bank-of-england/news-decomp.git
cd news-decomp
```
...
- Ruff's formatter
- API documentation generation from the public exports in `src/news_decomp`
...
Regenerate the Markdown export for the Marimo example when it changes:

```bash
python docs/convert_notebooks.py
```

Check that the committed notebook documentation is current:
